...
/* x210 led: IfanTsai add start */
/***************************************************************/
#define S5PV210_LED_PLATDATA(num) \
	static struct s5pv210_led_platdata x210_led##num##_pdata = {   \
		.name   = "led"#num,                                          \
		.gpio   = S5PV210_GPJ0(num + 2),                              \
	}

#define S5PV210_LED_DEVICE(num)     \
	static struct platform_device s5pv210_led##num = {      \
		.name	= "s5pv210-led",                             \
		.id		= num - 1,                                   \
		.dev	= {                                          \
			.platform_data	= &x210_led##num##_pdata,        \
		},                                                   \
	}

S5PV210_LED_PLATDATA(1);
S5PV210_LED_PLATDATA(2);
S5PV210_LED_PLATDATA(3);

S5PV210_LED_DEVICE(1);
S5PV210_LED_DEVICE(2);
S5PV210_LED_DEVICE(3);

/***************************************************************/
/* x210 led: IfanTsai add end */
