...
static struct s3c_gpio_chip s5pv210_gpio_4bit[] = {
	{
		.chip	= {
			.base	= S5PV210_GPA0(0),               // 当前GPIO端口的基编号
			.ngpio	= S5PV210_GPIO_A0_NR,            // 当前GPIO端口的IO口数量
			.label	= "GPA0",                        // 当前端口的名字
			.to_irq = s5p_gpiolib_gpioint_to_irq,    // 当前端口中IO口换算成对应中断号的函数
		},
	}, {
		.chip	= {
			.base	= S5PV210_GPA1(0),
...
{
	struct s3c_gpio_chip *chip = s5pv210_gpio_4bit;  // chip数组，描述所有GPIO端口信息
	int nr_chips = ARRAY_SIZE(s5pv210_gpio_4bit);      // GPIO端口数量
	int i = 0;
