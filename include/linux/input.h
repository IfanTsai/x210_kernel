...
// 所有输入设备的输入事件的抽象
struct input_event {
	struct timeval time;    // 输入事件发生的时间点
	__u16 type;             // 输入设备类型
	__u16 code;             // 具体事件编码
	__s32 value;            // 该具体事件的值
};
...
#define EV_SYN			0x00     // 同步数据
#define EV_KEY			0x01     // 按键 
#define EV_REL			0x02     // 相对坐标（如鼠标移动，提交的是相对最后一次位置的偏移）
#define EV_ABS			0x03     // 绝对坐标（如触摸屏和操作杆，提交的是绝对的坐标位置）
...
// 提交同步事件，告知input子系统，该设备已经提交了一个完整报告
