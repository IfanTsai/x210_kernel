...
struct platform_device {
	const char	* name;          // 名字 
	int		id;
	struct device	dev;         // 所有设备通用部分的抽象
	u32		num_resources;       // 使用的资源个数
	struct resource	* resource;  // 使用的资源数组的首地址

	const struct platform_device_id	*id_entry;  // 指向设备ID表

	/* arch specific additions */
	struct pdev_archdata	archdata;  // 特定架构下的属性，预留属性
};
...
struct platform_driver {
	...
	int (*resume)(struct platform_device *);      // 设备唤醒函数
	struct device_driver driver;                  // 所有驱动通用部分的抽象
