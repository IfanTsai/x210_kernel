...
struct evdev {
	...
	struct evdev_client *grab;      // 管理该事件驱动下的所有evdev_client
			  ...
			  size_t count, loff_t *ppos)
{
	struct evdev_client *client = file->private_data;
	struct evdev *evdev = client->evdev;
	struct input_event event;   // 用来描述一个底层设备上报的事件
	int retval;
    ...
    // evdev_fetch_next_event函数从client->buffer中取出从底层设备提交上来的事件赋给event,
    // 然后将这个event传递给从用户层传下来的buffer，完成从事件驱动层到用户空间的数据传递
